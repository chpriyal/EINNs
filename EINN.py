...
class EINN(nn.Module):
    def __init__(
        ...
        # print(train_dataset.rmse.shape)
        if(self.noise):
        # Adding noise to the training data
            if(self.noise == 'all'):
                for ind2 in range(train_dataset.rmse.shape[2]):
                    for ind1 in range(train_dataset.rmse.shape[0]):
                        numel = (train_dataset.rmse[ind1,:,ind2] != -9).sum()
                        noise = np.random.normal(0,self.stdev ,numel)
                        train_dataset.rmse[ind1,:noise.shape[0],ind2]+=noise
                        # noise = noise[1:]
            else:
                if self.noise== "S":
                    ind2 = 0
                ...
                if self.noise== "M":
                    ind2 = 4
                # noise = np.random.normal(0,self.stdev ,train_dataset.rmse.shape[1])
                for ind1 in range(train_dataset.rmse.shape[0]):
                    numel = (train_dataset.rmse[ind1,:,ind2] != -9).sum()
                    noise = np.random.normal(0,self.stdev ,numel)
                    train_dataset.rmse[ind1,:noise.shape[0],ind2]+=noise
            print("Noise added to {} with stand dev = {}".format(self.noise, self.stdev))
        # print(train_dataset.rmse[0])
        # print("Second")
        # print("Second")
        # print(train_dataset.rmse[1])
        # exit()
